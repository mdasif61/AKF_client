...
const useReaction = (reaction) => {
    ...
    const {data:single_react, refetch, isLoading:reactLoading}=useQuery(['single-reaction',users?._id],
    async()=>{
        try {
            const res=await axiosSecure.get(`/single-reaction/${users?._id}?reaction=${keys}`);
            return res.data;
        } catch (error) {
